...
describe('主角编译式网格计划', () => {
  ...
  it('按编译语义写入皮肤、衣物、围巾、金属和确定性分面变化', () => {
    ...
    const metal = evaluatePlan(
      fixture.state,
      VANGUARD_METAL_MESH_PLAN,
      new VanguardMeshEvaluator(VANGUARD_METAL_MESH_PLAN, VANGUARD_METAL_MESH_PALETTE),
      MeshDirty.All,
    );
    ...
    const scarf = getColor(matte, getSpan(VANGUARD_MATTE_MESH_PLAN, VanguardMatteSurface.Scarf)
      .startVertex);
    const steel = getColor(metal, getSpan(VANGUARD_METAL_MESH_PLAN, VanguardMetalSurface.Steel)
      .startVertex);
    const brass = getColor(metal, getSpan(VANGUARD_METAL_MESH_PLAN, VanguardMetalSurface.Brass)
      .startVertex);
  ...
  it('计划求值不会重写固定索引或语义数据', () => {
    ...
    const indices = Array.from(VANGUARD_METAL_MESH_PLAN.indices);
    const semantics = Array.from(VANGUARD_METAL_MESH_PLAN.semanticIds);

    evaluatePlan(fixture.state, VANGUARD_METAL_MESH_PLAN, evaluator, MeshDirty.All);
    fixture.animation.update(fixture.state, 0.25);
    evaluatePlan(fixture.state, VANGUARD_METAL_MESH_PLAN, evaluator, MeshDirty.Pose);

    expect(Array.from(VANGUARD_METAL_MESH_PLAN.indices)).toEqual(indices);
    expect(Array.from(VANGUARD_METAL_MESH_PLAN.semanticIds)).toEqual(semantics);
  });
});
