...
export function createCrystalClusterMeshPlan(): BattlefieldEnvironmentMeshPlan {
  ...
  appendRockMass(sink, 0, 0, 0, 1.25, 301, ROCK_BASE);
  appendCrystalShard(sink, -0.2, 0.38, 0.06, -0.35, 3.55, 0.18, 0.58, 313, CRYSTAL_CORE);
  appendCrystalShard(sink, 0.48, 0.28, -0.28, 0.82, 2.55, -0.5, 0.42, 331, CRYSTAL_BRIGHT);
  appendCrystalShard(sink, -0.58, 0.24, -0.36, -0.92, 2.05, -0.62, 0.36, 347, CRYSTAL_DEEP);
  appendCrystalShard(sink, 0.72, 0.22, 0.32, 1.04, 1.75, 0.58, 0.3, 359, CRYSTAL_CORE);
  return sink.build();
}
...
export function createRockFormationMeshPlan(): BattlefieldEnvironmentMeshPlan {
  const sink = new BattlefieldEnvironmentMeshSink();
  appendRockMass(sink, 0, 0, 0, 1.45, 373, ROCK_BASE);
  appendIrregularTube(sink, ROCK_MOSS, Object.freeze([
