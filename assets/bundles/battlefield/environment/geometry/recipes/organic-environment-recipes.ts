...
export function createDeadTreeMeshPlan(): BattlefieldEnvironmentMeshPlan {
  ...
  appendIrregularTube(sink, TREE_BARK, Object.freeze([
  ...
  appendBranch(sink, -0.15, 3.15, 0.05, -1.2, 4.65, 0.45, 31);
  appendBranch(sink, 0.05, 3.75, -0.02, 1.35, 5.05, -0.55, 47);
  appendBranch(sink, 0.2, 4.55, 0.08, -0.62, 5.82, -0.85, 59);
  appendCanopy(sink, -1.18, 4.92, 0.48, 1.35, 71, TREE_CANOPY_DARK);
  appendCanopy(sink, 1.38, 5.32, -0.52, 1.55, 83, TREE_CANOPY);
  appendCanopy(sink, 0.02, 6.18, 0.1, 1.7, 97, TREE_CANOPY);
  return sink.build();
}
...
export function createLuminousMushroomMeshPlan(): BattlefieldEnvironmentMeshPlan {
  const sink = new BattlefieldEnvironmentMeshSink();
  appendIrregularTube(sink, MUSHROOM_STEM, Object.freeze([
  ...
  appendIrregularTube(sink, MUSHROOM_GILL, Object.freeze([
    ring(0.08, 1.67, 0.02, 0.72, 0.64, 0.08),
    ring(0.04, 1.86, 0.01, 1.08, 0.94, -0.02),
    ring(-0.02, 2.15, 0.03, 1.22, 1.05, 0.12),
  ]), 9, 137);
  appendIrregularTube(sink, MUSHROOM_CAP, Object.freeze([
...
export function createGlowPlantMeshPlan(): BattlefieldEnvironmentMeshPlan {
  const sink = new BattlefieldEnvironmentMeshSink();
  appendGlowStem(sink, 0, 0, 0, -0.12, 1.55, 0.08, 173);
  appendGlowStem(sink, -0.22, 0, 0.06, -0.72, 1.08, 0.18, 181);
  appendGlowStem(sink, 0.18, 0, -0.08, 0.68, 1.22, -0.26, 193);
  appendLeaf(sink, -0.08, 0.46, 0.02, -0.8, 0.82, 0.34);
  appendLeaf(sink, 0.05, 0.68, -0.03, 0.82, 0.92, -0.32);
  appendLeaf(sink, -0.02, 0.88, 0.02, -0.48, 1.18, -0.58);
  appendBulb(sink, -0.12, 1.66, 0.08, 0.32, 211);
  appendBulb(sink, -0.74, 1.18, 0.2, 0.25, 223);
  appendBulb(sink, 0.7, 1.31, -0.28, 0.28, 227);
  return sink.build();
}
...
function appendBranch(
  ...
  appendIrregularTube(sink, TREE_BARK_DARK, Object.freeze([
...
function appendCanopy(
  ...
  appendIrregularTube(sink, color, Object.freeze([
...
function appendGlowStem(
  ...
  appendIrregularTube(sink, GLOW_STALK, Object.freeze([
...
function appendBulb(
  ...
  appendIrregularTube(sink, GLOW_BULB, Object.freeze([
