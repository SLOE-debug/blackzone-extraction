...
/** 管理运行时屏幕空间 UI 独占的 Canvas、正交相机和窗口尺寸同步。 */
export class ScreenUiCanvas {
  ...
  constructor(parent: Node, name: string) {
    ...
    try {
      ...
      const camera = createUiCamera(node, `${name}Camera`);
...
function createUiCamera(parent: Node, name: string): Camera {
  const cameraNode = new Node(name);
...
function synchronizeUiCanvasFrame(node: Node, transform: UITransform): void {
  ...
  if (transform.width !== visibleSize.width || transform.height !== visibleSize.height) {
    transform.setContentSize(visibleSize.width, visibleSize.height);
  }
  if (node.position.x !== centerX || node.position.y !== centerY) {
