...
export class GeometrySectionComposer<TSection extends string> {
  ...
  constructor(private readonly writer: TriangleMeshWriter) {}
  ...
  public write(section: TSection, appendGeometry: () => void): GeometrySectionRange {
    ...
    const startVertex = this.writer.vertexCount;
    const startIndex = this.writer.indexCount;
    appendGeometry();
    const range = Object.freeze({
      startVertex,
      vertexCount: this.writer.vertexCount - startVertex,
      startIndex,
      indexCount: this.writer.indexCount - startIndex,
    });
